...
class CellMetric(BaseModel):
    ...
    stack_id: str = Field(pattern=r"^[A-Z]$")  # Allow more stacks
    cell_id: int = Field(ge=1, le=100)  # Allow more cells
    ...
    sensor_quality: Optional[float] = Field(default=1.0, ge=0.0, le=1.0)
...
class KafkaToQuestDB:
    ...
    def __init__(
        ...
        kafka_topic: str = "flux.cell.metrics.v1",  # <<< CHANGED: Now a single topic
        kafka_group: str = "questdb-writer",
        questdb_config: Dict[str, Any] = None,
    ):
        self.kafka_brokers = kafka_brokers
        self.kafka_topic = kafka_topic  # <<< CHANGED
    ...
    async def start(self):
        ...
        self.running = True
        logger.info("Kafka consumer started", topic=self.kafka_topic)
...
async def main():
    ...
    pipeline = KafkaToQuestDB(
        ...
        # Topic is now hardcoded to the canonical one, can also be env var
        questdb_config={
            "host": questdb_host,
            "port": questdb_port,
            "batch_size": 10000,
